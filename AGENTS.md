...
├── src/whatsapp_mcp/       # Python MCP server — exposes tools to AI clients
│   ├── server.py           # FastMCP tool definitions + main() entry point
│   ├── whatsapp.py         # DB queries + bridge HTTP client (+ WhatsAppDBError)
│   ├── audio.py            # FFmpeg helpers
│   └── config.py           # Centralized OS-aware paths + env var overrides
...
5. **Tests.** Add or update tests for any code you touch in `src/whatsapp_mcp/`. The Go bridge has fewer tests today; matching the existing bar is fine, but don't *remove* coverage.
...
# Python MCP server (run from repo root)
pip install -e ".[dev]"
whatsapp-mcp                # dev — or `python -m whatsapp_mcp`
pytest -m "not integration" # tests (unit only)
ruff check .                # lint
ruff format .               # format
mypy src                    # type-check
```
...
|---|---|
| Add or modify an MCP tool | `src/whatsapp_mcp/server.py` |
| Change DB queries / data conversion | `src/whatsapp_mcp/whatsapp.py` |
