...
3. **Configure Claude Desktop**
   ...
   The `whatsapp-mcp` console script is installed by `pip install -e .` (or
   `pipx install .`) from the repo root. If you'd rather not install editable,
   ...
   Replace `/path/to/whatsapp-mcp` with your actual repo path.
...
| **MCP server** (`src/whatsapp_mcp/*.py`, `pyproject.toml`) | Restart Claude Desktop / Cursor. If you installed with `-e`, edits are picked up immediately; otherwise re-run `pip install -e .` to refresh the entry point. |
