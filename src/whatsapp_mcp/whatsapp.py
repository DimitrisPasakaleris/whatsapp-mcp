...
class WhatsAppDBError(RuntimeError):
    """Raised when a SQLite query against the bridge databases fails.

    Distinguishes a real database failure from an empty result set so the
    MCP layer can surface a structured error to the client instead of an
    indistinguishable empty list.
    """


# Bridge HTTP timeout. The bridge is local but uploads of medium-sized media
# files can take a few seconds, so keep this generous.
...
MESSAGES_DB_PATH = str(config.messages_db_path)
WHATSMEOW_DB_PATH = str(config.whatsmeow_db_path)
WHATSAPP_API_BASE_URL = config.bridge_api_url
...
def _get_chat_messages_ordered(chat_jid: str) -> list[Message]:
    ...
    try:
        ...
        logger.error("Database error in _get_chat_messages_ordered: %s", e)
        raise WhatsAppDBError(f"_get_chat_messages_ordered failed: {e}") from e
    finally:
        conn.close()
...
def list_messages(
    ...
    try:
        ...
        if include_context and result:
            # Bulk-fetch context to avoid N+1 queries.
            # Group matched messages by chat, fetch each chat's full message list
            # once (1 query per chat vs 3 queries per matched message), then
            # extract context windows by index.
            ...
            for chat_jid, chat_matches in by_chat.items():
                all_chat_msgs = _get_chat_messages_ordered(chat_jid)
        ...
        raise WhatsAppDBError(f"list_messages failed: {e}") from e
    finally:
        if "conn" in locals():
            conn.close()
...
def list_chats(
    ...
    try:
        ...
        logger.error("Database error: %s", e)
        raise WhatsAppDBError(f"Database query failed: {e}") from e
    finally:
        if "conn" in locals():
            conn.close()
...
def _post_to_bridge(payload: dict[str, Any]) -> tuple[bool, str]:
    """POST a payload to the bridge ``/send`` endpoint and translate the response.

    Centralizes timeout, error mapping, and response parsing for all three
    ``send_*`` tools so a fix only has to happen once. Always returns
    ``(success, message)`` — never raises.
    """
    url = f"{WHATSAPP_API_BASE_URL}/send"
    try:
        response = _session.post(url, json=payload, timeout=HTTP_TIMEOUT_SECONDS)
...
def send_message(recipient: str, message: str) -> tuple[bool, str]:
    ...
    return _post_to_bridge({"recipient": recipient, "message": message})
...
def send_file(recipient: str, media_path: str) -> tuple[bool, str]:
    ...
    return _post_to_bridge({"recipient": recipient, "media_path": media_path})
...
def send_audio_message(recipient: str, media_path: str) -> tuple[bool, str]:
    ...
    if not media_path.endswith(".ogg"):
        try:
            ...
            return False, (
                "Error converting file to opus ogg. You likely need to install ffmpeg: "
                f"{e}"
            )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if result.get("success", False):
                ...
                logger.error("Download failed: %s", result.get('message', 'Unknown error'))
                return None
        else:
            logger.error("Download HTTP error: %d - %s", response.status_code, response.text)
            return None
        ...
        logger.error("Download parse error: %s", response.text)
        return None
    except Exception as e:
        logger.error("Download unexpected error: %s", e)
        return None
