...
def read_server_json_versions() -> tuple[str, str]:
    ...
    for pkg in data.get("packages", []):
        if pkg.get("registryType") == "pypi" and pkg.get("identifier") == "whatsapp-mcp":
            package_version = pkg.get("version")
            break

    if not package_version:
        raise ValueError("server.json is missing the whatsapp-mcp PyPI package version")
...
def main() -> int:
    ...
    if pyproject_version != package_version:
        errors.append(
            f"Version mismatch: pyproject.toml={pyproject_version} "
            f"!= server.json.packages[whatsapp-mcp].version={package_version}"
        )
    ...
    if tag:
        try:
            ...
            if pyproject_version != normalized_tag:
                errors.append(
                    f"Version mismatch: tag={normalized_tag} != pyproject.toml={pyproject_version}"
                )
