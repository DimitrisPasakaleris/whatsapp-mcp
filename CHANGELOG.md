...
### Changed
- **Repository layout**: Python server moved from flat `whatsapp-mcp-server/` modules
  to a proper src-layout package at `src/whatsapp_mcp/`. The package is now
  pip-installable (`pip install -e .`) and registers a `whatsapp-mcp` console
  script plus `python -m whatsapp_mcp` module entry point.
- **Error handling**: Database errors in `whatsapp.py` now raise
  `WhatsAppDBError` instead of silently returning `[]` / `None`. The MCP layer
  catches this and surfaces a structured `{"error": "database_error", ...}`
  response so callers can distinguish a real failure from "no results".
- **HTTP timeouts**: All `_session.post(...)` calls to the Go bridge now use
  `timeout=30s`, preventing the server from hanging indefinitely on a wedged
  bridge.
- **send_*  dedup**: `send_message`, `send_file`, and `send_audio_message`
  share a single `_post_to_bridge` helper. Eliminates the prior triple
  duplication and the latent `UnboundLocalError` in the `json.JSONDecodeError`
  branches.
- **Go webhook logs**: `webhook.go` lines now carry a `[webhook]` prefix so
  they're attributable in interleaved bridge stdout.

### Added
- `WhatsAppDBError` exception (`whatsapp_mcp.whatsapp.WhatsAppDBError`).
- `tests/test_db_errors.py` covering the new error path on six DB-backed tools.
- `tests/integration/test_smoke.py` parametrized over all 13 tools.
- MIT `LICENSE`, `manifest.json` (MCPB), `py.typed` marker.
- `mypy` config + CI job; `pytest --cov`; coverage upload artifact.
