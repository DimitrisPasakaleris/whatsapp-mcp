...
# MCP server (separate terminal, from repo root)
pip install -e ".[dev]"
whatsapp-mcp
```
...
4. **Test locally** — `pytest -m "not integration"`, `ruff check .`, `mypy src`, `golangci-lint run`, `go build ./...`.
5. **Open a PR** to `main`. Use the PR template.
